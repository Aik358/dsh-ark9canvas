      ...
      function renderGenerate() {
        ...
        if (!st.cfg.baseURL && !(st.cfg.channels && st.cfg.channels.length)) {
          body.appendChild(el('div', 'background:' + WARN_BG + ';border:1px solid rgba(255,193,7,0.4);color:#ffe08a;padding:8px 10px;border-radius:8px;font-size:12px;margin-bottom:10px;',
            '未配置渠道。请到 设置 → Ark9 生图 添加渠道(baseURL 含 /v1、Key、模型)。'))
        }
        // 模型行
        var modelRow = el('div', 'display:flex;gap:8px;align-items:center;margin-bottom:10px;')
        var modelSel = document.createElement('select')
        modelSel.style.cssText = 'flex:1;padding:6px;border-radius:8px;border:1px solid rgba(255,255,255,0.14);background:rgba(255,255,255,0.08);color:inherit;font-size:12px;'
        function fillModels() {
          modelSel.innerHTML = ''
          var list = st.models.length ? st.models : (st.cfg && st.cfg.model ? [{ id: st.cfg.model, name: st.cfg.model }] : [])
          if (!list.length) {
            var o = document.createElement('option'); o.textContent = '(未取到模型,可在设置页拉取)'; modelSel.appendChild(o); return
          }
          list.forEach(function (m) {
            var o = document.createElement('option'); o.value = m.id; o.textContent = m.name || m.id
            modelSel.appendChild(o)
          })
          var cur = st.model || (st.cfg && st.cfg.model) || ''
          if (cur && list.some(function (m) { return m.id === cur })) modelSel.value = cur
        }
        fillModels()
        modelSel.onchange = function () { st.model = modelSel.value }
        modelRow.appendChild(modelSel)
        modelRow.appendChild(miniBtn('刷新模型', '', function () {
          apiPost(API.models, {}).then(function (r) {
            if (r.models && r.models.length) { st.models = r.models; fillModels() }
          }).catch(function () {})
        }))
    ...
    function SettingsPage() {
      ...
      function addChannel() {
        setCfg(function (prev) {
          ...
          chs.push({ id: id, name: '渠道 ' + (chs.length + 1), baseURL: '', apiKey: '', model: 'gpt-image-2' })
          o.channels = chs
      ...
      return h('div', { style: { padding: 20, maxWidth: 620, color: FG, fontFamily: 'system-ui, sans-serif' } },
        ...
        ch ? h('div', null,
          ...
          h('label', { style: labelStyle }, 'BaseURL(含 /v1)',
            h('input', { value: ch.baseURL || '', onChange: function (e) { updCh(chIdx, 'baseURL', e.target.value) }, placeholder: 'https://sub.vankit.top/v1', style: fieldStyle })),
          ...
          h('label', { style: labelStyle }, '默认模型',
            h('div', { style: { display: 'flex', gap: 8 } },
              h('input', { value: ch.model || '', onChange: function (e) { updCh(chIdx, 'model', e.target.value) }, placeholder: 'gpt-image-2', style: Object.assign({ flex: 1 }, fieldStyle) }),
