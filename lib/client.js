...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function mountPanelContent(container, opts) {
      ...
      langBtn.style.cssText = 'background:transparent;border:1px solid rgba(255,255,255,0.18);border-radius:999px;color:inherit;opacity:.75;cursor:pointer;font-size:11px;padding:2px 8px;'
      ...
      function miniBtn(text, cssExtra, onclick, iconName) {
        var b = el('button', 'display:inline-flex;align-items:center;gap:5px;justify-content:center;padding:5px 10px;border-radius:8px;border:1px solid rgba(255,255,255,0.18);background:rgba(255,255,255,0.08);color:inherit;cursor:pointer;font-size:12px;' + (cssExtra || ''))
      ...
      function renderGenerate() {
        ...
        if (!st.cfg.baseURL && !(st.cfg.channels && st.cfg.channels.length)) {
          body.appendChild(el('div', 'background:' + WARN_BG + ';border:1px solid rgba(255,193,7,0.4);color:#ffe08a;padding:8px 10px;border-radius:8px;font-size:12px;margin-bottom:10px;', t('noConfigWarn')))
        }
        ...
        function refresh() {
          ...
          apiGet(API.approvals).then(function (r) {
            ...
            list.forEach(function (a) {
              ...
              btnRow.appendChild(miniBtn(t('approve'), 'flex:1;background:' + OK_BG + ';color:#a5d6a7;font-weight:600;border:0;', function () { decide(a.id, 'approve') }, 'check'))
              btnRow.appendChild(miniBtn(t('deny'), 'flex:1;background:' + ERR_BG + ';color:#ff8a80;font-weight:600;border:0;', function () { decide(a.id, 'deny') }, 'x'))
      ...
      function renderPrompts() {
        ...
        function renderList() {
          ...
          if (st.promptSource === '_local') {
            ...
            if (st.promptFetchError) listBox.appendChild(el('div', 'font-size:11px;color:#ff8a80;margin-bottom:4px;', t('lastFetch') + st.promptFetchError))
      ...
      function renderHistory() {
        ...
        toolbar.appendChild(miniBtn(t('deleteSel'), 'color:#ff8a80;', function () {
        ...
        toolbar.appendChild(miniBtn(t('clearLogs'), 'color:#ff8a80;', function () {
    ...
    function SettingsPage() {
      ...
      return h('div', { style: { padding: 20, maxWidth: 620, color: FG, fontFamily: 'system-ui, sans-serif' } },
        h('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
          ...
          h('button', { onClick: function () { setLocale(loc === 'zh' ? 'en' : 'zh') }, style: { padding: '2px 10px', borderRadius: 999, border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'inherit', cursor: 'pointer', fontSize: 11 } }, t('langBtn'))),
        ...
        h('div', { style: { display: 'flex', gap: 6, marginBottom: 12 } },
          ...
          (ch && cfg.channels.length > 1) ? h('button', { onClick: function () { delChannel(chIdx); setChIdx(0) }, style: Object.assign({}, smallBtn, { color: '#ff8a80' }) }, t('delChannel')) : null),
        ch ? h('div', null,
        ...
        (cfg.promptSources || []).map(function (s, i) {
          return h('div', { key: s.id || i, style: { display: 'flex', gap: 6, marginBottom: 6 } },
            ...
            h('button', { onClick: function () { var arr = cfg.promptSources.slice(); arr.splice(i, 1); upd('promptSources', arr) }, style: Object.assign({}, smallBtn, { color: '#ff8a80' }) }, t('del')))
        }),
