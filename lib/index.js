...
export const GUIDANCE = '本机已安装 dsh-ark9canvas-pre 插件（生图工作台）：调用 ark9_generate_image 生成图片（文生图/图生图/透明背景；images 可传参考图 dataURL 或之前生成的本地文件路径做多轮迭代；count 一次最多 10 张）。生成结果保存到输出目录并返回文件路径+尺寸，默认不内嵌 base64（returnDataUrl=true 才内嵌，占大量上下文慎用）。ark9_list_images 列出最近生成。**审批纪律（默认模式）**：调用 ark9_generate_image 后，生图请求会进入审批队列等待用户批准——工具会立即返回"等待审批"状态，**你必须马上在回复中告诉用户：「生图请求已发出，请到右下角 🖼️ Ark9 生图 → 审批 页批准（或拒绝），批准后会自动生成并在这里汇报」**，不要等待、不要重复调用、不要假装已生成。审批结果（批准后生成的图片路径 / 用户拒绝 / 超时取消）会在后续轮次以 <ark9_approval_notice> 注入，看到后向用户汇报：批准 → 汇报图片路径与效果；用户拒绝 → 不要重试同一提示词，询问用户想怎么改；超时取消 → 提醒用户如需要可重新发起。设置页可将审批改为 never（免审批，直接生成并返回结果，正常汇报）。用户说「画一张/生成图片/配图/把这张图改成…」时即用本插件，不要编造图片链接。'
